...
export const ComponentCustom = ({
    ...
    if(loading === true) {
        ...
        if(data[queryObject.query_tablename].length === 0) {
            ...
            ToRender = <Grid container spacing={2}>{data[queryObject.query_tablename].map((item, itemIndex) => queryObject.forEachItem(item, itemIndex, () => {
            ...
            }, setShowExpanded))}</Grid>
        }
    } else {
        ToRender = <ErrorText errorText={queryObject.null_found}/>
    }
